...
-(void)testResourceFilter{
	...
	STAssertTrue([filter shouldPass:TEST_PATH], @"Must be passed");
}
...
-(void)testResourceExtensionFilter{
	...
	STAssertFalse([filter shouldPass:TEST_PATH], @"Mustn't be passed");
}
...
-(void)testResourceExtensionFilterWithExtensions{
	...
	STAssertTrue([filter shouldPass:TEST_PASS_PATH], @"Must be passed");
	STAssertTrue([filter shouldPass:TEST_PASS_PATH1], @"Must be passed");
	STAssertFalse([filter shouldPass:TEST_SHOULD_NOT_PASS_PATH], @"Mustn't be passed");
	STAssertFalse([filter shouldPass:TEST_SHOULD_NOT_PASS_PATH1], @"Mustn't be passed");
	STAssertFalse([filter shouldPass:TEST_SHOULD_NOT_PASS_PATH2], @"Mustn't be passed");
}
